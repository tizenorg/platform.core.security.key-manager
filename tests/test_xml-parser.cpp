...
BOOST_AUTO_TEST_CASE(XmlParserTest_wrong_argument) {
	...
	BOOST_REQUIRE(Parser::ErrorCode::PARSE_SUCCESS == parser.RegisterElementCb("Key",
				  dummyStartCallback, dummyEndCallback));
...
BOOST_AUTO_TEST_CASE(XmlParserTest_no_XML_file) {
	...
	BOOST_REQUIRE(Parser::ErrorCode::ERROR_XML_VALIDATION_FAILED == parser.Validate(format_test_path(
					  XSD_1_okay).c_str()));
}
...
BOOST_AUTO_TEST_CASE(XmlParserTest_XML1_wrong_schema) {
	XML::Parser parser(format_test_path(XML_1_okay).c_str());
	BOOST_REQUIRE(Parser::ErrorCode::ERROR_XSD_PARSE_FAILED == parser.Validate(format_test_path(
					  XSD_1_wrong).c_str()));
}
...
BOOST_AUTO_TEST_CASE(XmlParserTest_XML1_correct_parse) {
	...
	BOOST_REQUIRE(0 == parser.Validate(format_test_path(XSD_1_okay).c_str()));
	BOOST_REQUIRE(Parser::ErrorCode::PARSE_SUCCESS == parser.RegisterElementCb("Key",
				  dummyStartCallback, NULL));
	BOOST_REQUIRE(Parser::ErrorCode::PARSE_SUCCESS == parser.RegisterElementCb("Cert", NULL,
				  dummyEndCallback));
...
BOOST_AUTO_TEST_CASE(XmlParserTest_XML4_device_key_correct_parse) {
	...
	BOOST_REQUIRE(Parser::ErrorCode::PARSE_SUCCESS == parser.RegisterElementCb("RSAPrivateKey",
				  dummyStartCallback, NULL));
	startCallbackFlag = false;
	BOOST_REQUIRE(Parser::ErrorCode::PARSE_SUCCESS == parser.Parse());
	BOOST_REQUIRE(startCallbackFlag == true);

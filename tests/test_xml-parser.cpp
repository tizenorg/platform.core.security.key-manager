...
{
    XML::Parser parser(0);
    BOOST_REQUIRE(XML::Parser::ERROR_INVALID_ARGUMENT == parser.Validate(0));
    BOOST_REQUIRE(XML::Parser::ERROR_XML_PARSE_FAILED == parser.Parse());
}

BOOST_AUTO_TEST_CASE(XmlParserTest_no_XML_file)
{
    XML::Parser parser(format_test_path("i-am-not-here").c_str());
    BOOST_REQUIRE(XML::Parser::ERROR_XML_VALIDATION_FAILED == parser.Validate(format_test_path(XSD_1_okay).c_str()));
}
...
{
    XML::Parser parser(format_test_path(XML_1_okay).c_str());
    BOOST_REQUIRE(XML::Parser::ERROR_XSD_PARSE_FAILED == parser.Validate(format_test_path(XSD_1_wrong).c_str()));
}
    ...
    class MathHandler : public XML::Parser::ElementHandler
    {
        public:
            ...
            virtual void Start(const XML::Parser::Attributes &attr) {
                if(attr.find("powerFactor") != attr.end())
                    m_powerFactor = atoi(attr.at("powerFactor").c_str());
            }
            virtual void Characters(const std::string &data) {

...
void DBFixture::performance_stop(long num_operations_performed) {
	...
	BOOST_TEST_MESSAGE("\t<performance> time elapsed: " << time_elapsed_ms << "[ms], number of " <<
					   m_operation << ": " << num_operations_performed);

	if (num_operations_performed > 0)
		BOOST_TEST_MESSAGE("\t<performance> average time per " << m_operation << ": " << time_elapsed_ms /
						   num_operations_performed << "[ms]");
}

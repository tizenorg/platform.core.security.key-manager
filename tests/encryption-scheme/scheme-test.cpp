...
void restoreFile(const string& filename) {
    ...
    err = errno;
    BOOST_REQUIRE_MESSAGE(sourceFd > 0, "Opening " << sourcePath << " failed: " << err);

    FdPtr sourceFdPtr(&sourceFd);

    int targetFd = TEMP_FAILURE_RETRY(creat(targetPath.c_str(), 0644));
    err = errno;
    BOOST_REQUIRE_MESSAGE(targetFd > 0, "Creating " << targetPath << " failed: " << err);

    ret = fchown(targetFd, CKM_UID, CKM_GID);
    err = errno;
    BOOST_REQUIRE_MESSAGE(ret != -1, "fchown() failed: " << err);
    ...
    err = errno;
    BOOST_REQUIRE_MESSAGE(ret != -1, "fstat() failed: " << err);

    ret = sendfile(targetFd, sourceFd, 0, sourceStat.st_size);
    err = errno;
    BOOST_REQUIRE_MESSAGE(ret != -1, "sendfile() failed: " << err);

    ret = fsync(targetFd);
    err = errno;
    BOOST_REQUIRE_MESSAGE(ret != -1, "fsync() failed: " << err);

...
void SchemeTest::ReadAll() {
    ...
    for(int i=0;i<AFTER_LAST;i++) {
        ...
        case DataType::BINARY_DATA:
        {
            ...
            ret = m_mgr->getData(ITEMS[i].alias, ITEMS[i].policy.password, receivedData);
            BOOST_REQUIRE_MESSAGE(receivedData == TEST_DATA,
        ...
        case DataType::KEY_RSA_PUBLIC:
        {
            ...
            ret = m_mgr->getKey(ITEMS[i].alias, ITEMS[i].policy.password, receivedKey);
            break;
        }
        ...
        case DataType::CERTIFICATE:
        {
            ...
            ret = m_mgr->getCertificate(ITEMS[i].alias, ITEMS[i].policy.password, receivedCert);
            break;
        }
        ...
        {
            PKCS12ShPtr pkcs;
            ret = m_mgr->getPKCS12(ITEMS[i].alias,
                                   ITEMS[i].policy.password,
                                   ITEMS[i].policy.password,
                                   pkcs);
            break;
        }
    ...
    for(int i=FIRST;i<AFTER_LAST;i++) {
        ...
        for(const auto& r : rows) {
            BOOST_REQUIRE_MESSAGE(((r.encryptionScheme & NEW_ENC_SCHEME) > 0) == isNew,
                                  ...
                                  << NEW_ENC_SCHEME << " got: " << r.encryptionScheme);
        }
    }
}

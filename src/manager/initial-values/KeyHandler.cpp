...
CKM::KeyType KeyHandler::parseType(const std::string & typeStr)
{
    if     (typeStr == XML_ATTR_TYPE_VAL_RSA_PRV)      return CKM::KeyType::KEY_RSA_PRIVATE;
    else if(typeStr == XML_ATTR_TYPE_VAL_RSA_PUB)      return CKM::KeyType::KEY_RSA_PUBLIC;
    else if(typeStr == XML_ATTR_TYPE_VAL_DSA_PRV)      return CKM::KeyType::KEY_DSA_PRIVATE;
    else if(typeStr == XML_ATTR_TYPE_VAL_DSA_PUB)      return CKM::KeyType::KEY_DSA_PUBLIC;
    else if(typeStr == XML_ATTR_TYPE_VAL_ECDSA_PRV)    return CKM::KeyType::KEY_ECDSA_PRIVATE;
    else if(typeStr == XML_ATTR_TYPE_VAL_ECDSA_PUB)    return CKM::KeyType::KEY_ECDSA_PUBLIC;
    else if(typeStr == XML_ATTR_TYPE_VAL_AES)          return CKM::KeyType::KEY_AES;
    else // should not happen
        throw std::runtime_error("error: invalid value discovered as key type");
}

CKM::DataType KeyHandler::getDataType() const
{
    return CKM::DataType(m_keyType);
}

...
int Parser::Validate(const char *XSD_schema)
{
    ...
    int retCode;
    xmlSchemaParserCtxtPtr parserCtxt = xmlSchemaNewParserCtxt(XSD_schema);
    if(parserCtxt == NULL) {
    ...
    retCode = xmlSchemaValidateFile(validCtxt, m_XMLfile.c_str(), 0);
    if(0 != retCode) {
...
void Parser::Error(const ErrorType errorType, const char *msg, va_list &args)
{
    if(m_errorCb)
    {
        ...
        va_list args2;
        va_copy(args2, args1);
        std::vector<char> buf(1 + std::vsnprintf(NULL, 0, msg, args1));
        va_end(args1);

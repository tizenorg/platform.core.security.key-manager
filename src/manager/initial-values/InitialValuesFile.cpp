...
InitialValuesFile::InitialValuesFile(const char *XML_filename)
        ...
        const std::string & log_msg)
{
    ...
    switch(errorType)
    {
        case LOG_LEVEL_WARNING:
            LogWarning("parsing " << m_XMLfile << " warning: " << log_msg);
            break;
        default:
            LogWarning("parsing " << m_XMLfile << " error: " << log_msg);
            break;
    }
#else
    std::cerr << __func__ << " type " << errorType << ": " << log_msg << std::endl;
#endif
}

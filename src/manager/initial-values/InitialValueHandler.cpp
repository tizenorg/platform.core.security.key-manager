...
void InitialValueHandler::End()
{
    ...
    {
        // save data
        CKM::Policy policy(m_password, m_exportable);
        int ec = m_db_logic->saveData(
                Credentials(CKMLogic::SYSTEM_DB_UID, LABEL_SYSTEM_DB),
                m_name,
                LABEL_SYSTEM_DB,
                m_bufferHandler.get()->getData(),
                getDataType(),
        ...
        if(CKM_API_SUCCESS == ec)
        {
            ...
            for(const auto & permission : m_permissions)
            {
                ec = m_db_logic->setPermissionHelper(
                        ...
                        m_name,
                        LABEL_SYSTEM_DB,
                        permission.get()->getAccessor(),
                        Permission::READ);
                if(CKM_API_SUCCESS != ec)
                    LogError("Saving permission to: " << m_name << " with params: accessor("<<permission.get()->getAccessor()<<") failed, code: " << ec);
            }
        }
        else
        ...
        LogError("Invalid key with name: " << m_name << ", reason: no key data!");
}

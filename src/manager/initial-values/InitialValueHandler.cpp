...
void InitialValueHandler::Start(const XML::Parser::Attributes &attr)
{
    ...
    if(attr.find(XML_ATTR_NAME) != attr.end())
        m_name = CKM::Alias(attr.at(XML_ATTR_NAME));

    // get password
    if(attr.find(XML_ATTR_PASSWORD) != attr.end())
        m_password = CKM::Password(attr.at(XML_ATTR_PASSWORD).c_str());
...
void InitialValueHandler::End()
{
    if(m_bufferHandler)
    {
        ...
        int ec = m_db_logic.verifyAndSaveDataHelper(
                CKM::Credentials(CKM::CKMLogic::SYSTEM_DB_UID, CKM::LABEL_SYSTEM_DB),
                m_name,
                CKM::LABEL_SYSTEM_DB,
                m_bufferHandler->getData(),
                getDataType(),
                CKM::PolicySerializable(policy));

...
ParamTypeMap _ckmc_init_param_type_map()
{
    ParamTypeMap map;
    map[CKMC_PARAM_ALGO_TYPE] = INTEGER;

    map[CKMC_PARAM_ED_IV] = BUFFER;
    map[CKMC_PARAM_ED_CTR_LEN] = INTEGER;
    map[CKMC_PARAM_ED_AAD] = BUFFER;
    map[CKMC_PARAM_ED_TAG_LEN] = INTEGER;
    map[CKMC_PARAM_ED_LABEL] = BUFFER;

    map[CKMC_PARAM_GEN_KEY_LEN] = INTEGER;
    map[CKMC_PARAM_GEN_EC] = INTEGER;

    map[CKMC_PARAM_SV_HASH_ALGO] = INTEGER;
    map[CKMC_PARAM_SV_RSA_PADDING] = INTEGER;
    return map;
}

ParamTypeMap g_paramTypeMap = _ckmc_init_param_type_map();
...
template <typename T>
void _ckmc_set_param_value(ckmc_param_s *param, T value)
{
    param->integer = value;
}

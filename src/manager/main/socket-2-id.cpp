...
int getPkgIdFromSmack(const std::string &smack, std::string &pkgId) {
    ...
    static const std::string SMACK_SYSTEM_USER   = "User";
    static const std::string SMACK_SYSTEM_SYSTEM = "System";

    if (SMACK_SYSTEM_USER == smack) {
        pkgId = PKGID_USER;
        return 0;
    }

    if (SMACK_SYSTEM_SYSTEM == smack) {
        pkgId = PKGID_SYSTEM;
        return 0;
    }

    if (smack.compare(0, SMACK_PREFIX_APPID.size(), SMACK_PREFIX_APPID)) {
        LogError("Unsupported smack label: " << smack);
        return -1;
    }

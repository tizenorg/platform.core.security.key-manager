...
KeyImplShPtr CertificateImpl::getKeyImpl() const {
    EvpShPtr evp(X509_get_pubkey(m_x509), EVP_PKEY_free);
    switch(EVP_PKEY_type(evp->type))
    {
        case EVP_PKEY_RSA:
            return KeyBuilder::create(evp, KeyType::KEY_RSA_PUBLIC);
        case EVP_PKEY_DSA:
            return KeyBuilder::create(evp, KeyType::KEY_DSA_PUBLIC);
        case EVP_PKEY_EC:
            return KeyBuilder::create(evp, KeyType::KEY_ECDSA_PUBLIC);
        default:

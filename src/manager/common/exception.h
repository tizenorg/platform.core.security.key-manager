...
class COMMON_API DefineException : public Exception {
  public:
	...
	DefineException(const char *path, const char *function, int line, const Args &... args)
		...
		Before(m_path, m_function, m_line, DefineException<Error, Stringify, Before, After>::error(),
			   m_message);
	}
	~DefineException() noexcept {
		After(m_path, m_function, m_line, DefineException<Error, Stringify, Before, After>::error(),
			  m_message);
	}

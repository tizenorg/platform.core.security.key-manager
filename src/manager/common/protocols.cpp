...
PKCS12Serializable::PKCS12Serializable(IStream &stream) {
	...
	if (keyPresent) {
		...
		if (m_pkey)
			LogDebug("private key from pkcs deserialized success. key size: " << keyData.size() <<
					 " and DER size: " << m_pkey->getDER().size());
		else
	...
	if (certPresent) {
		...
		if (m_cert)
			LogDebug("certificate from pkcs deserialized success. cert size: " << certData.size() <<
					 " and DER size: " << m_cert->getDER().size());
		else
	...
	for (size_t i = 0; i < numCA; i++) {
		...
		if (m_pkey)
			LogDebug("ca certificate from pkcs deserialized success. cert size: " << CAcertData.size() <<
					 " and DER size: " << CKM::Certificate::create(CAcertData, DataFormat::FORM_DER)->getDER().size());
		else

...
bool isAsym(const CKM::KeyType type)
{
    switch (type) {
...
bool isPrivate(const CKM::KeyType type)
{
    switch (type) {
    case CKM::KeyType::KEY_RSA_PRIVATE:
    case CKM::KeyType::KEY_ECDSA_PRIVATE:
...
bool isValidType(const CKM::KeyType type,
                 const bool isPrivate,
                 const int pkeyType)
{
    switch (pkeyType) {
    case EVP_PKEY_RSA:
        if ((isPrivate && type == CKM::KeyType::KEY_RSA_PRIVATE)
            || (!isPrivate && type == CKM::KeyType::KEY_RSA_PUBLIC))
            return true;
    case EVP_PKEY_DSA:
        if ((isPrivate && type == CKM::KeyType::KEY_DSA_PRIVATE)
            || (!isPrivate && type == CKM::KeyType::KEY_DSA_PUBLIC))
            return true;
    case EVP_PKEY_EC:
        if ((isPrivate && type == CKM::KeyType::KEY_ECDSA_PRIVATE)
            || (!isPrivate && type == CKM::KeyType::KEY_ECDSA_PUBLIC))
            return true;
    default:
        break;
    }
...
CKM::KeyType getValidType(int pkeyType, int isPrivate)
{
    switch (pkeyType) {
    case EVP_PKEY_RSA:
        return isPrivate ?
            CKM::KeyType::KEY_RSA_PRIVATE
          : CKM::KeyType::KEY_RSA_PUBLIC;
    case EVP_PKEY_DSA:
        return isPrivate ?
            CKM::KeyType::KEY_DSA_PRIVATE
          : CKM::KeyType::KEY_DSA_PUBLIC;
    case EVP_PKEY_EC:
        return isPrivate ?
            CKM::KeyType::KEY_ECDSA_PRIVATE
          : CKM::KeyType::KEY_ECDSA_PUBLIC;
    default:
        return CKM::KeyType::KEY_NONE;
    }
}
...
void AsymKeyImpl::setBinary(EVP_PKEY *pkey)
{
    if (!isAsym(m_type)) {
        LogError("Invalid key type");
        return;
                     ...
                     const Password &password)
{
    try {
        ...
        if (!keyShPtr && type == KeyType::KEY_NONE)
            keyShPtr = std::make_shared<AsymKeyImpl>(keyBuffer, password);

...
CKM::RawBuffer i2d(I2D_CONV fun, EVP_PKEY* pkey) {
    ...
    if (NULL == pkey) {
        ThrowErr(CKM::Crypto::Exception::InternalError, "attempt to parse an empty key!");
    }

    if (NULL == bio.get()) {
        ThrowErr(CKM::Crypto::Exception::InternalError, "Error in memory allocation! Function: BIO_new.");
    }

    if (1 != fun(bio.get(), pkey)) {
        ThrowErr(CKM::Crypto::Exception::InternalError, "Error in conversion EVP_PKEY to DER");
    }
    ...
    if (size <= 0) {
        ThrowErr(CKM::Crypto::Exception::InternalError, "Error in BIO_read: ", size);
    }
...
int initialize() {
    ...
    if(u_rand_ret != 32 ){
        ...
        if(hw_rand_ret != 32) {
            ThrowErr(Exception::InternalError, "Error in U_RAND_file_load");
        }
    }
...
const EVP_MD *getMdAlgo(const HashAlgorithm hashAlgo) {
    ...
    case HashAlgorithm::SHA512:
         ...
         break;
    default:
        ThrowErr(Exception::InternalError, "Error in hashAlgorithm value");
    }
    return md_algo;
}
...
int getRsaPadding(const RSAPaddingAlgorithm padAlgo) {
    ...
    case RSAPaddingAlgorithm::X931:
        ...
        break;
    default:
        ThrowErr(Exception::InternalError, "Error in RSAPaddingAlgorithm value");
    }
    return rsa_padding;
}
...
TokenPair createKeyPairRSA(CryptoBackend backendId, const int size)
{
    ...
    if(size!=1024 && size!=2048 && size!=4096) {
        ThrowErr(Exception::InputParam, "Error in RSA input size");
    }

    EvpPkeyCtxUPtr ctx(EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL), EVP_PKEY_CTX_free);
    if(!ctx) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_new_id function !!");
    }

    if(EVP_PKEY_keygen_init(ctx.get()) <= 0) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_keygen_init function !!");
    }

    if(EVP_PKEY_CTX_set_rsa_keygen_bits(ctx.get(), size) <= 0) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_set_rsa_keygen_bits function !!");
    }

    EVP_PKEY *pkeyTmp = NULL;
    if(!EVP_PKEY_keygen(ctx.get(), &pkeyTmp)) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_keygen function !!");
    }
...
TokenPair createKeyPairDSA(CryptoBackend backendId, const int size)
{
    ...
    if(size!=1024 && size!=2048 && size!=3072 && size!=4096) {
        ThrowErr(Exception::InputParam, "Error in DSA input size");
    }
    ...
    if(EVP_SUCCESS != EVP_PKEY_paramgen_init(pctx.get())) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_paramgen_init function");
    }

    if(EVP_SUCCESS != EVP_PKEY_CTX_set_dsa_paramgen_bits(pctx.get(), size)) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_set_dsa_paramgen_bits(", size, ") function");
    }

    /* Generate parameters */
    EVP_PKEY *pparamTmp = NULL;
    if(EVP_SUCCESS != EVP_PKEY_paramgen(pctx.get(), &pparamTmp)) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_paramgen function");
    }
    ...
    if(!kctx) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_new function");
    }
...
TokenPair createKeyPairECDSA(CryptoBackend backendId, ElipticCurve type)
{
    ...
    case ElipticCurve::secp384r1:
        ...
        break;
    default:
        ThrowErr(Exception::InputParam, "Error in EC type");
    }
    ...
    if(EVP_SUCCESS != EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx.get(), ecCurve)) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_set_ec_paramgen_curve_nid function");
    }
        ...
        const int rsa_padding)
{
    ...
    if(EVP_PKEY_sign_init(pctx.get()) != EVP_SUCCESS) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_sign_init function");
    }
    ...
    if(EVP_PKEY_type(privKey->type) == EVP_PKEY_RSA) {
        if(EVP_SUCCESS != EVP_PKEY_CTX_set_rsa_padding(pctx.get(), rsa_padding)) {
            ThrowErr(Exception::InternalError,
    ...
    if(EVP_SUCCESS != EVP_PKEY_sign(pctx.get(), NULL, &slen, message.data(), message.size())) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_sign function");
    }
    ...
    ThrowErr(Exception::InputParam, "Error in EVP_PKEY_sign function. Input param error.");
}
        ...
        const int rsa_padding)
{
    ...
    if(!mdctx.get()) {
        ThrowErr(Exception::InternalError, "Error in EVP_MD_CTX_create function");
    }

    if(EVP_SUCCESS != EVP_DigestSignInit(mdctx.get(), &pctx, md_algo, NULL, privKey)) {
        ThrowErr(Exception::InternalError, "Error in EVP_DigestSignInit function");
    }

    /* Set padding algorithm */
    if(EVP_PKEY_type(privKey->type) == EVP_PKEY_RSA) {
        if(EVP_SUCCESS != EVP_PKEY_CTX_set_rsa_padding(pctx, rsa_padding)) {
            ThrowErr(Exception::InternalError, "Error in EVP_PKEY_CTX_set_rsa_padding function");
        }
    }

    /* Call update with the message */
    if(EVP_SUCCESS != EVP_DigestSignUpdate(mdctx.get(), message.data(), message.size())) {
        ThrowErr(Exception::InternalError, "Error in EVP_DigestSignUpdate function");
    }
    ...
    if(EVP_SUCCESS != EVP_DigestSignFinal(mdctx.get(), NULL, &slen)) {
        ThrowErr(Exception::InternalError, "Error in EVP_DigestSignFinal function");
    }
...
//    auto shrPKey = publicKey.getEvpShPtr();
    if (NULL == pkey) {
        ThrowErr(Exception::InternalError, "Error in getEvpShPtr function");
    }
        ...
        const int rsa_padding)
{
    ...
    if(EVP_PKEY_verify_init(pctx.get()) != EVP_SUCCESS) {
        ThrowErr(Exception::InternalError, "Error in EVP_PKEY_verify_init function");
    }
        ...
        const int rsa_padding)
{
    ...
    if(EVP_SUCCESS != EVP_DigestVerifyInit(mdctx.get(), &pctx, md_algo, NULL, pubKey)) {
        ThrowErr(Exception::InternalError, "Error in EVP_DigestVerifyInit function");
    }
    ...
    if(EVP_SUCCESS != EVP_DigestVerifyUpdate(mdctx.get(), message.data(), message.size()) ) {
        ThrowErr(Exception::InternalError, "Error in EVP_DigestVerifyUpdate function");
    }

    ...
    CKM::ParamName paramName)
{
    ...
    if (!alg.getParam(paramName, result)) {
        ...
        ThrowMsg(CKM::Crypto::Exception::InputParam, "Wrong input param");
    }
    return result;
}
    ...
    int tagSize)
{
    if (tagSize > static_cast<int>(data.size()))
        ThrowMsg(Crypto::Exception::InputParam, "Wrong size of tag");
                           ...
                           const RawBuffer &data)
{
    ...
    if(false == alg.getParam(ParamName::ALGO_TYPE, keyType))
        ThrowMsg(Crypto::Exception::InputParam,
            "symmetric enc error: mandatory parameter ALGO_TYPE not specified");
    ...
    ThrowMsg(Crypto::Exception::OperationNotSupported,
                           ...
                           const RawBuffer &data)
{
    ...
    if(false == alg.getParam(ParamName::ALGO_TYPE, keyType))
        ThrowMsg(Crypto::Exception::InputParam,
            "symmetric dec error: mandatory parameter ALGO_TYPE not specified");

...
bool verifyBinaryData(DataType dataType, const RawBuffer &buffer)
{
    if (dataType.isSKey()) {
        switch (buffer.size()) {
            default:
                LogDebug("AES key have wrong size.");
                return false;
            case 128:
            case 192:
            case 256:
    ...
    if (dataType.isKeyPublic() || dataType.isKeyPrivate()) {
        BioUniquePtr bio(BIO_new(BIO_s_mem()), BIO_free_all);
        ...
        LogDebug("Key was not verified. Unsupported format.");
        return false;
    }
    ...
    if (dataType.isCertificate() || dataType.isChainCert())
    {
        ...
        LogDebug("Certificate was not verified. Unsupported format.");
        return false;
    }

...
DataPair createKeyPairDSA(const int size) {
	...
	if (EVP_SUCCESS != EVP_PKEY_CTX_set_dsa_paramgen_bits(pctx.get(), size))
		ThrowErr(Exc::Crypto::InternalError, "Error in EVP_PKEY_CTX_set_dsa_paramgen_bits(", size,
				 ") function");
...
int verifyMessage(EVP_PKEY *pubKey,
	...
	if (EVP_SUCCESS == EVP_PKEY_verify(pctx.get(), signature.data(), signature.size(), message.data(),
									   message.size()))
...
int digestVerifyMessage(EVP_PKEY *pubKey,
	...
	if (EVP_SUCCESS == EVP_DigestVerifyFinal(mdctx.get(), const_cast<unsigned char *>(signature.data()),
			signature.size()))
		return CKM_API_SUCCESS;

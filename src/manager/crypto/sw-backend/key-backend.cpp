...
#include <sw-backend/key-backend.h>
...
int createKeyPairDSA(const int size, // size in bits [1024, 2048, 3072, 4096]
		KeyImpl &createdPrivateKey,  // returned value
		KeyImpl &createdPublicKey)  // returned value
{
...
int createKeyPairECDSA(ElipticCurve type,
        KeyImpl &createdPrivateKey,  // returned value
        KeyImpl &createdPublicKey)  // returned value
{
...
//    }
    ...
    if(md_algo == NULL) {
        retCode = signMessage(pkey, message, rsa_padding, signature);
    }else {
        retCode = digestSignMessage(pkey,message, md_algo, rsa_padding, signature);
    }

    if (retCode != CKM_API_SUCCESS) {
        Throw(Crypto::Exception::InternalError);
    }
...
//    auto shrPKey = publicKey.getEvpShPtr();
    ...
    if (md_algo == NULL) {
        retCode = verifyMessage(pkey, message, signature, rsa_padding);
    } else {
        retCode = digestVerifyMessage(pkey, message, signature, md_algo, rsa_padding);
    }

...
public:
    ...
    EvpCipherWrapper(const EVP_CIPHER *type, const T &key, const T &iv, bool encryption)
    {
        if (static_cast<int>(key.size()) != EVP_CIPHER_key_length(type)) {
            ThrowErr(CKM::Crypto::Exception::InternalError, "Wrong key size! Expected: ", EVP_CIPHER_key_length(type) ," Get: ", key.size());
        }

        if (static_cast<int>(iv.size()) < EVP_CIPHER_iv_length(type)) {
            ThrowErr(CKM::Crypto::Exception::InternalError, "Wrong iv size! Expected: ", EVP_CIPHER_iv_length(type) , " Get: ", iv.size());
        }

        if (1 != EVP_CipherInit_ex(m_ctx, type, NULL, key.data(), iv.data(), encryption ? 1 : 0)) {
            ThrowErr(CKM::Crypto::Exception::InternalError, "Failed in EVP_CipherInit");
        }
    ...
    T Append(const T& data) {
        ...
        if (1 != EVP_CipherUpdate(m_ctx, output.data(), &bytesLen, data.data(), data.size())) {
            ThrowErr(CKM::Crypto::Exception::InternalError, "Failed in EVP_CipherUpdate");
        }
    ...
    T Finalize() {
        ...
        if (1 != EVP_CipherFinal_ex(m_ctx, output.data(), &bytesLen)) {
            ThrowErr(CKM::Crypto::Exception::InternalError, "Failed in EVP_CipherFinal");
        }
        output.resize(bytesLen);
        return output;
    }

...
Token Store::importEncrypted(DataType dataType,
                             ...
                             const RawBuffer &encryptedAESKey,
                             const RawBuffer &IVforAES)
{
    if(m_deviceKey)
    {
        ...
        Crypto::SW::SKey AES_key = Crypto::SW::SKey(m_deviceKey->decrypt(algorithmRSAOAEP, encryptedAESKey), DataType::KEY_AES);
        ...
        algorithmAESCBC.setParam(ParamName::ED_IV, IVforAES);

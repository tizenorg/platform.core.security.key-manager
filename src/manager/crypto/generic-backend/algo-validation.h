...
typedef std::shared_ptr<ParamCheckBase> ParamCheckBasePtr;
 ...
 * Getter - gets the value (value itself or a buffer size for example)
    ...
    void Check(const CryptoAlgorithm& ca) const {
        ...
        if(!ca.getParam(Name,value)) {
            if (Mandatory) {
                ...
                ErrorHandler::Handle(std::move(os.str()));
            }
            return;
        }
        // validate

    ...
    const Policy &policy)
{
    ...
    // Key format was checked in CAPI before,
    // but do double check in case of client used C++API directly
    if (!Key::create(key->getBinary(), key->getType()))
        return CKM_API_ERROR_INVALID_FORMAT;

    try {
        return saveBinaryData(alias, DataType(key->getType()), key->getBinary(), policy);
    } catch (const DataType::Exception::Base &e) {
        LogError("CKM::DataType::Exception " << e.DumpToString());
        return CKM_API_ERROR_INPUT_PARAM;
    }
}
...
int ManagerImpl::getKey(const Alias &alias, const Password &password, KeyShPtr &key)
{
    ...
    key = std::make_shared<KeyImpl>(rawData, static_cast<KeyType>(recvDataType));

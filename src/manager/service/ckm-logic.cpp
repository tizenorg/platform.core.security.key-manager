...
{
    Crypto::GStore& store = m_decider.getStore(data.getType(), policy.extractable);
...
int CKMLogic::verifyBinaryData(DataContainer &input_data) const
{
    DataContainer dummy;
    return toBinaryData(input_data, dummy);
}

int CKMLogic::toBinaryData(const DataContainer &input_data,
                           DataContainer &output_data) const
{
    ...
    if (input_data.getType().isKey())
    {
        ...
        if(input_data.getType().isSKey())
            output_key = CKM::Key::createAES(input_data.getData());
        else
            output_key = CKM::Key::create(input_data.getData());
        if(output_key.get() == NULL)
        {
            LogDebug("provided binary data is not valid key data");
            return CKM_API_ERROR_INPUT_PARAM;
        }
        output_data = DataContainer(input_data.getType(), output_key->getDER());
    }
    else if (input_data.getType().isCertificate() || input_data.getType().isChainCert())
    {
        CertificateShPtr cert = CKM::Certificate::create(input_data.getData(), DataFormat::FORM_DER);
        if(cert.get() == NULL)
        {
            LogDebug("provided binary data is not valid certificate data");
            return CKM_API_ERROR_INPUT_PARAM;
        }
        output_data = DataContainer(input_data.getType(), cert->getDER());
    }
    else
    ...
    DB::RowVector &output) const
{
    ...
    DataContainer keyData(DataType(keyPtr->getType()), keyPtr->getDER());
    ...
    if( !pkcs.getCertificate() )
        return CKM_API_ERROR_INVALID_FORMAT;
    DataContainer certData(DataType::CERTIFICATE, pkcs.getCertificate().get()->getDER());
    ...
    {
        DataContainer caCertData(DataType::getChainDatatype(cert_index ++), ca->getDER());
    ...
    const PolicySerializable &policy)
{
    ...
    return saveDataHelper(cred,
                          name,
                          label,
                          DataContainer(DataType::KEY_AES, key.data),
                          policy);
}
    ...
    const PolicySerializable &policyPublic)
{
    ...
    int retCode = saveDataHelper(cred,
                             namePrivate,
                             labelPrivate,
                             DataContainer(keys.first.dataType, keys.first.data),
                             policyPrivate);
    if (CKM_API_SUCCESS != retCode)
        return retCode;
    ...
    retCode = saveDataHelper(cred,
                             namePublic,
                             labelPublic,
                             DataContainer(keys.second.dataType, keys.second.data),
                             policyPublic);
    if (CKM_API_SUCCESS != retCode)
        return retCode;

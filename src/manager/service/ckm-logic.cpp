        ...
        const Label &label)
{
    ...
    retCode = handler.loadAppKey(rows.front().ownerLabel);
    if(CKM_API_SUCCESS != retCode)
        return retCode;
    ...
    DB::RowVector &rows)
{
    ...
    // load app key if needed
    retCode = handler.loadAppKey(firstRow.ownerLabel);
    if(CKM_API_SUCCESS != retCode)
        return retCode;
    ...
    DB::Row &row)
{
    ...
    // load app key if needed
    retCode = handler.loadAppKey(row.ownerLabel);
    if(CKM_API_SUCCESS != retCode)
        return retCode;
...
int UserData::loadAppKey(const Label& appLabel)
{
    if (!crypto.haveKey(appLabel)) {
        RawBuffer key;
        auto key_optional = database.getKey(appLabel);
        if(!key_optional) {
            LogError("No key for given label in database");
            return CKM_API_ERROR_DB_ERROR;
        }
        key = *key_optional;
        key = keyProvider.getPureDEK(key);
        crypto.pushKey(appLabel, key);
    }
    return CKM_API_SUCCESS;
}

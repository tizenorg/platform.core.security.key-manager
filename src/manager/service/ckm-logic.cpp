...
int CKMLogic::readDataHelper(
	...
	// check access rights
	retCode = checkDataPermissionsHelper(cred, name, ownerLabel, cred.smackLabel, firstRow, exportFlag,
										 handler.database);
...
int CKMLogic::readDataHelper(
	...
	// check access rights
	retCode = checkDataPermissionsHelper(cred, name, ownerLabel, cred.smackLabel, row, exportFlag,
										 handler.database);
...
int CKMLogic::getPKCS12Helper(
	...
	retCode = readDataHelper(true, cred, DataType::DB_CHAIN_FIRST, name, label, certPassword,
							 caChainObjs);
...
int CKMLogic::saveDataHelper(
	...
	retCode = extractPKCS12Data(handler.crypto, name, ownerLabel, pkcs, keyPolicy, certPolicy,
								encryptedRows);
...
RawBuffer CKMLogic::createSignature(
	...
	try {
		Crypto::GObjUPtr obj;
		retCode = readDataHelper(false, cred, DataType::DB_KEY_FIRST, privateKeyName, ownerLabel, password,
								 obj);
...
RawBuffer CKMLogic::verifySignature(
	...
	try {
		...
		Crypto::GObjUPtr obj;
		retCode = readDataHelper(false, cred, DataType::CERTIFICATE, publicKeyOrCertName, ownerLabel,
								 password, obj);

		if (retCode == CKM_API_ERROR_DB_ALIAS_UNKNOWN)
			retCode = readDataHelper(false, cred, DataType::DB_KEY_FIRST, publicKeyOrCertName, ownerLabel,
									 password, obj);

...
int CKMLogic::verifyBinaryData(DataType dataType, const RawBuffer &input_data) const
{
    ...
    if (dataType.isKey())
    {
        KeyImplShPtr output_key = CKM::KeyBuilder::create(input_data);
    ...
    DB::RowVector &output) const
{
    ...
    RawBuffer keyData = keyImplPtr->getInternalBinary();
    int retCode = verifyBinaryData(keyType, keyData);
    if(retCode != CKM_API_SUCCESS)
        return retCode;
    output.push_back(createRow(name, ownerLabel, keyType, keyImplPtr->getInternalBinary(), keyPolicy));
    ...
    CertificateShPtrVector & caChain)
{
    ...
    if(retCode != CKM_API_SUCCESS)
        return retCode;
    privKey = CKM::KeyBuilder::create(privKeyRow.data);
    ...
    const PolicySerializable &policyPublic)
{
    ...
    KeyImplShPtr pPrv = KeyBuilder::createNullAsymKey();
    KeyImplShPtr pPub = KeyBuilder::createNullAsymKey();
    int retCode;
    switch(key_type)
    {
        ...
        const RSAPaddingAlgorithm padding)
{
    ...
    try {
        ...
        if(retCode == CKM_API_SUCCESS)
        {
            KeyImplShPtr keyParsed = KeyBuilder::create(row.data);
            if (keyParsed->empty())
                retCode = CKM_API_ERROR_SERVER_ERROR;
            else
    ...
    try {
        do {
            ...
            if (retCode == CKM_API_SUCCESS) {
                ...
                if (retCode != CKM_API_SUCCESS)
                    break;
                key = KeyBuilder::create(row.data);
            } else {
                break;
            }

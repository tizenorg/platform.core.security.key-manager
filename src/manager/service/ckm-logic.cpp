...
{
    Label effLabel;
    auto &handler = selectDatabase(cred, ownerLabel, effLabel);

    // use client label if not explicitly provided
    if(effLabel.empty())
        effLabel = cred.smackLabel;
    ...
    PermissionMaskOptional permissionRowOpt =
            handler.database.getPermissionRow(name, effLabel, cred.smackLabel);
    ...
    auto erased = handler.database.deleteRow(name, effLabel);
...
{
    Label effLabel;
    auto &handler = selectDatabase(cred, label, effLabel);

    // use client label if not explicitly provided
    const Label &ownerLabel = effLabel.empty() ? cred.smackLabel : effLabel;
...
{
    Label effLabel;
    auto &handler = selectDatabase(cred, labelPrivate, effLabel);

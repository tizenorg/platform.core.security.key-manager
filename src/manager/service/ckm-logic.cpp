    ...
    const Password& password)
{
    ...
    // decrypt row
    Crypto::GObjUPtr obj;
    if(CryptoLogic::newScheme(row.encryptionScheme)) {
    ...
    Crypto::GObjUPtrVector &objs)
{
    ...
    Crypto::GStore& store = m_decider.getStore(dataType, exportFlag);

    // decrypt row
    for(auto &row : rows) {
        objs.push_back(rowToObject(handler, store, std::move(row), password));
    }
    ...
    DataType& objDataType)
{
    ...
    obj = rowToObject(handler, store, std::move(row), password);
    // rowToObject may modify db
    transaction.commit();
        ...
        CertificateImplVector &certVector)
{
    ...
    for (auto &i: labelNameVector) {
        ...
        Crypto::GObjUPtr obj;
        int ec = readDataHelper(true, cred, DataType::CERTIFICATE, i.second, i.first, Password(), obj);
        ...
        Crypto::GObjUPtrVector caChainObjs;
        ec = readDataHelper(true, cred, DataType::DB_CHAIN_FIRST, i.second, i.first, CKM::Password(), caChainObjs);

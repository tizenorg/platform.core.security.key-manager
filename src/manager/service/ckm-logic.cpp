    ...
    const PolicySerializable &policy)
{
    ...
    return saveDataHelper(cred,
                          name,
                          label,
                          Crypto::Data(DataType::KEY_AES, std::move(key.data)),
                          policy);
}
    ...
    const PolicySerializable &policyPublic)
{
    ...
    int retCode = saveDataHelper(cred,
                             namePrivate,
                             labelPrivate,
                             Crypto::Data(keys.first.dataType, std::move(keys.first.data)),
                             policyPrivate);
    if (CKM_API_SUCCESS != retCode)
        return retCode;
    ...
    retCode = saveDataHelper(cred,
                             namePublic,
                             labelPublic,
                             Crypto::Data(keys.second.dataType, std::move(keys.second.data)),
                             policyPublic);
    if (CKM_API_SUCCESS != retCode)
        return retCode;

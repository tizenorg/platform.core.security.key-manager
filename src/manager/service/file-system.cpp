...
{
    std::stringstream ss;
    ss << CKM_DATA_DIR << CKM_DB_PREFIX << m_uid;
    return ss.str();
}
...
{
    std::stringstream ss;
    ss << CKM_DATA_DIR << CKM_KEY_PREFIX << m_uid;
    return ss.str();
}
...
{
    std::stringstream ss;
    ss << CKM_DATA_DIR << CKM_DB_KEY_PREFIX << m_uid;
    return ss.str();
}
...
{
    std::stringstream ss;
    ss << CKM_DATA_DIR << CKM_REMOVED_APP_PREFIX << m_uid;
    return ss.str();
}
...
{
    errno = 0;
    if ((mkdir(CKM_DATA_DIR, 0700)) && (errno != EEXIST)) {
        int err = errno;
        LogError("Error in mkdir " << CKM_DATA_DIR << ". Reason: " << GetErrnoString(err));
...
UidVector FileSystem::getUIDsFromDBFile()
{
    ...
    std::unique_ptr<DIR, std::function<int(DIR*)>>
        dirp(::opendir(CKM_DATA_DIR), ::closedir);
    ...
    size_t len = offsetof(struct dirent, d_name) + pathconf(CKM_DATA_DIR, _PC_NAME_MAX) + 1;

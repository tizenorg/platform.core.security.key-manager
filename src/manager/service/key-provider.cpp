...
RawBuffer KeyProvider::getPureDomainKEK() {
	...
	return RawBuffer(m_kmcDKEK->getKeyAndInfo().key,
					 (m_kmcDKEK->getKeyAndInfo().key) + m_kmcDKEK->getKeyAndInfo().keyInfo.keyLength);
}
...
int KeyProvider::encryptAes256Gcm(const unsigned char *plaintext, int plaintext_len,
...
int KeyProvider::decryptAes256Gcm(const unsigned char *ciphertext, int ciphertext_len,
								  unsigned char *tag, const unsigned char *key, const unsigned char *iv, unsigned char *plaintext) {
	EVP_CIPHER_CTX *ctx;
	int len;
	int plaintext_len;
	int ret;

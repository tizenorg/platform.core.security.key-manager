...
const std::string OLD_SS_DIR = "/opt/share/secure-storage";
	...
	while (true) {
		...
		if (result->d_type == DT_DIR) {
			if ((name_size == 1 && name[0] == '.') ||
				...
				visit_dir(subdir, buf, ++depth, saver);
				if (::rmdir(subdir.c_str()) == -1)
					LogError("Failed to rmdir dir: " << subdir << " with errno: " << errno);
			}
			...
			if (depth == 0) {
				...
				auto p = dirpath.rfind(storage_name);
				Crypto::Data data;
				...
				if (data.data.empty()) {
					...
					// data is not in group! smack-label is used as storage name
					saver(storage_name, name, data);
				} else {
					// data is in group! group id is extracted from dir path
					saver(storage_name.substr(p + OLD_SS_GROUP_PREFIX.length()), name, data);
				}

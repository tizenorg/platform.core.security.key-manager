...
typedef struct __ckmc_algo_param_aes_gcm {
    ...
    unsigned char* iv;                  /**< initialization vector - up to 2^64-1 bytes long */
    unsigned char* aad;                 /**< optional additional auth data */
...
typedef struct __ckmc_algo_param_rsa_oaep {
    ...
    unsigned char* label;               /**< optional label to associate with the data */
...
/**
 * @brief Sets up default algorithm parameters for a given algorithm type
 *
 * @since_tizen 3.0
 *
 * @remarks Caller is responsible for ckmc_encryption_algo_s creation and destruction. Algorithm
 *          parameters used for encryption could be then used for decryption.
 * @remarks Algorithm parameters are set to default values. Optional fields are left empty.
 *          Initialization vectors are randomly generated. The function allocates algorithm params
 *          with malloc. Caller is responsible for freeing them.
 * @remarks Calling this function twice without freeing ckmc_encryption_algo contents will result in
 *          memory leak.
 *
 ...
 */
int ckmc_setup_algo(ckmc_algo_type_e type, ckmc_encryption_algo_s* algo);

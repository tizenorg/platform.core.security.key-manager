...
/**
 * @brief Structure for algorithm parameter name and value.
 * @since_tizen 3.0
 */
typedef struct __ckmc_param {
 ...
 * @since_tizen 3.0
 */
typedef struct __ckmc_param_list {
    ckmc_param_s *param;
    struct __ckmc_param_list *next;
} ckmc_param_list_s;
 ...
 * @param[in] buffer    Value of the parameter in form of a integer.
 *
 * @return @c 0 on success, otherwise a negative error value
 *
 * @retval #CKMC_ERROR_NONE                 Successful
 ...
 *                      creating the buffer. After calling this function the list takes ownership of
 *                      the buffer.
 *
 * @return @c 0 on success, otherwise a negative error value
 *
